...
const variantClasses = {
    ...
    "login": "border border-gray-500 text-white bg-blue-500 hover:bg-blue-600",
}

const defaultStyle = "p-2 rounded-md flex justify-center items-center transition duration-200 ease-in-out";

export default function Button({text, variant, onClick, disabled = false, extraclasses}){
    return (
        <button onClick={onClick} className={`${defaultStyle} ${extraclasses} ${disabled ? variantClasses.disabled : variantClasses[variant]}`}>
            {text}
        </button>
    )
}